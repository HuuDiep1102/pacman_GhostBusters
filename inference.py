...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        for p in self.legalPositions:
                #kiem tra xem co con ma nao bi an chua
            if noisyDistance == None:
                jail = self.getJailPosition()
                allPossible[jail] = 1.0
            else:
                trueDistance = util.manhattanDistance(p, pacmanPosition)
                #cap nhat belief dua tren trueDistance
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        self.particles = list()
        # danh sach vi tri cac bong ma co the den
        legalPositions = self.legalPositions
        # cai dat cac hat vi tri
    ...
    def observe(self, observation, gameState):
        ...
        allPossible = util.Counter()
        # neu pacman an ma thi vi tri hat la vi tri tu cua ma 
        if noisyDistance == None:
                self.particles = [self.getJailPosition() for i in range(self.numParticles)]
        else:
           # neu khong thi niem tin se dk cap nhat theo trueDistance
    ...
    def elapseTime(self, gameState):
        ...
        for i in range(self.numParticles):
            p = self.particles[i]
            # lay ra vi tri cua tat ca ma
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, p))
            # lay mau 1 so vi tri cho hat
